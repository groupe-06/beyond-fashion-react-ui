...
const UserProfil = ({ user, closePanel, isOpen }) => {
    ...
    return (
      <div className={`fixed top-0 right-0 w-1/4 bg-gray-900 text-white h-full z-50 transform transition-transform duration-300 ease-in-out ${isOpen ? 'translate-x-0' : 'translate-x-full'}`}>
        <div className="flex justify-between px-6 pt-6">
          <div className="flex items-center space-x-4 mb-6 p-6">
            <img src={user.avatar} alt={user.name} className="w-24 h-24 rounded-full border-2 border-purple-500 p-0.5" />
            <div>
        ...
        <div className="mt-20 bg-white rounded-t-3xl overflow-hidden h-screen p-10 mr-3">
          ...
          <div className="grid grid-cols-4 gap-2 mt-2">
            {user.portfolio && user.portfolio.map((img, index) => (
              <img
                key={index}
                src={img}
                alt="Portfolio image"
